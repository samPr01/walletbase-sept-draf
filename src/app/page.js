  ...
  useEffect(() => {
    const handleUserCreation = async () => {
      if (walletAddress && !userId) {
        ...
        updateUser(newUserId, walletAddress);
  ...
  const fetchAndUpdatePrices = async () => {
    try {
      ...
      if (data && data.success && data.prices) {
        ...
        // Calculate USD balances with null checks
        const newUsdBalances = {};
        let totalUSD = 0;
        
        if (balances && typeof balances === 'object') {
          Object.keys(balances).forEach(token => {
            if (data.prices[token] && balances[token] !== null && balances[token] !== undefined) {
              const balance = parseFloat(balances[token]) || 0;
              const price = data.prices[token].usd || 0;
              const usdValue = balance * price;
              newUsdBalances[token] = usdValue;
              totalUSD += usdValue;
            } else {
              newUsdBalances[token] = 0;
            }
          });
        }
